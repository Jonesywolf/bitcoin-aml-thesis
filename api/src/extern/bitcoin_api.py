...
def get_wallet_data_from_api(base58_address: str) -> Tuple[WalletData, ConnectedWallets]:
    """
...
def convert_to_wallet_data(address_query_response: BitcoinAddressQueryResponse) -> Tuple[WalletData, ConnectedWallets]:
    ...
    if not blocks_btwn_input_txs:
        ...
        blocks_btwn_input_txs_mean = blocks_btwn_input_txs_total / len(blocks_btwn_input_txs)
        blocks_btwn_input_txs_median = sorted(blocks_btwn_input_txs)[len(blocks_btwn_input_txs) // 2]
    ...
    if not blocks_btwn_output_txs:
        ...
        blocks_btwn_output_txs_mean = blocks_btwn_output_txs_total / len(blocks_btwn_output_txs)
        blocks_btwn_output_txs_median = sorted(blocks_btwn_output_txs)[len(blocks_btwn_output_txs) // 2]
    ...
    if total_txs == 0:
        ...
        transacted_w_address_median = sorted_num_addresses_transacted_with[total_txs // 2]
        transacted_w_address_min = min(num_addresses_transacted_with)
        transacted_w_address_max = max(num_addresses_transacted_with)
        num_addr_transacted_multiple = sum([1 for num in num_addresses_transacted_with if num > 1])      
